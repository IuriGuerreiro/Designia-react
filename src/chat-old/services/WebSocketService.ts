...
class WebSocketService {
  ...
  sendTypingStop(): boolean {
    if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
      console.warn('Cannot send typing_stop: WebSocket not connected');
      return false;
    }

    try {
      console.log('Sending typing_stop to server');
  ...
  markMessagesAsRead(): boolean {
    ...
    try {
      ...
      console.log('Mark messages as read sent successfully');
      return true;
    } catch (error) {
  ...
  private handleMessage(data: string): void {
    try {
      ...
      switch (message.type) {
        ...
        case 'typing_stop':
          if (message.user_id && message.username) {
            console.log(`Received typing_stop from ${message.username} (ID: ${message.user_id})`);
