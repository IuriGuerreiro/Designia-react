...
const ChatComponent: React.FC = () => {
  ...
  const handleTyping = useCallback(() => {
    ...
    // If not currently typing, send typing start (only once)
    if (!isTyping) {
      console.log('📝 Starting typing indicator for chat', selectedChat.id);
    ...
    typingTimeoutRef.current = setTimeout(() => {
      console.log('⏹️ Stopping typing indicator (timeout) for chat', selectedChat.id);
      sendTypingStop(selectedChat.id);
      setIsTyping(false);
      typingTimeoutRef.current = null;
    }, 2500);
  }, [isConnected, selectedChat, sendTypingStart, sendTypingStop, isTyping]);

  // Function to stop typing immediately (for message send)
  const stopTyping = useCallback(() => {
    if (isTyping && selectedChat) {
      console.log('⏹️ Stopping typing indicator (message sent) for chat', selectedChat.id);
      sendTypingStop(selectedChat.id);
      setIsTyping(false);
  ...
  }, [isTyping, selectedChat, sendTypingStop]);
